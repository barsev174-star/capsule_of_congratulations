...
Шаблон создан для семьи `universal-v1` и собирается через ателье. «Школьный классический» использован только как свежий архитектурный образец: семь секций, декларативный профиль, отдельные mobile/export-настройки, сменяемые фотографии и статус `studio`. Его художественные ассеты и предметная палитра не копировались.
