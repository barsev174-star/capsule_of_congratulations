...
3. Новые PNG/WebP `quote-card-*-export` шаблона воспитателя включены как подготовленные ассеты без переключения профиля. PDF из `output/`, секреты, временные файлы и пользовательские данные исключены.
