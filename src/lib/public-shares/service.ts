...
import type { PublicCardShare, PublicShareEditorInput, PublicSharePayloadV1 } from "./types";
...
export const getPublicShareEditor = async (finalSlug: string) => {
  ...
  const finalCard = buildFinalCardViewModel(card, contributions, mediaAssets, {
    qualities: qualitiesInsight?.items.map((item) => item.text)
  });
  const hasMomentsBlock = finalCard.blocks.some((block) => block.id === "memories");
  ...
  const publicQualities = finalCard.qualities.map((text, index) => ({ id: `quality-${index}`, text }));
  return { card, share, photos, mediaAssets: publicMediaAssets, phraseCandidates, publicQualities, wasRevoked, publicSharePath: share?.status === "ACTIVE" ? getPublicSharePath(createPublicShareToken(share.id)) : null, defaultDisplayName: firstName(card.recipientName) };
};
...
export const savePublicShare = async (finalSlug: string, submitted: PublicShareEditorInput) => {
  ...
  const share = editor.share ?? await createPublicShare({ id: createdShareId!, cardId: editor.card.id, tokenHash: null, displayName: editor.defaultDisplayName });
  const nextShare: PublicCardShare = {
    ...share, displayName: input.displayName, headlinePreset: input.headlinePreset, showOccasion: input.showOccasion,
  ...
  try {
    ...
    for (const [sortOrder, asset] of selectedAssets.entries()) {
      ...
      if (existing) {
        photoRows.push({ ...existing, publicShareId: share.id, sortOrder, publicCaption: clean(input.photoCaptions[asset.id], 120) || "" });
      } else {
        ...
        photoRows.push({ publicShareId: share.id, cardMediaAssetId: asset.id, ...derivative, mimeType: asset.mimeType, sortOrder, publicCaption: clean(input.photoCaptions[asset.id], 120) || "" });
      }
    }
...
export const getPublicSharePayload = async (token: string): Promise<PublicSharePayloadV1 | null> => {
