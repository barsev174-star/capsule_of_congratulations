...
const promptLeakagePatterns = [
  ...
  /(?<!\p{L})просто\s+и\s+по[\s-]*человечески(?!\p{L})/iu,
...
const repeatedTemplatePhrases = [
  { label: "дело, которым хочется заниматься", pattern: /дел\p{L}*\s+котор\p{L}*\s+хочет\p{L}*\s+занима/iu },
  { label: "доход, который радует", pattern: /доход\p{L}*\s+котор\p{L}*\s+раду/iu },
  { label: "возможности для роста", pattern: /возможност\p{L}*[\s\S]{0,24}(?:раст|рост)/iu },
  { label: "место, где тебя ценят", pattern: /мест\p{L}*\s+где[\s\S]{0,24}ценят/iu },
  { label: "похоже, диплом", pattern: diplomaJokePattern }
];
...
export const findSharedTemplatePhrase = (left: string, right: string) => {
  const normalizedLeft = normalizeComparable(left);
  const normalizedRight = normalizeComparable(right);
  return repeatedTemplatePhrases.find(
    ({ pattern }) => pattern.test(normalizedLeft) && pattern.test(normalizedRight)
  )?.label ?? null;
};
...
export const inspectProviderVariants = (input: ProviderValidationInput): ProviderVariantValidationResult => {
  ...
  for (let index = 0; index < Math.min(rawVariants.length, 3); index += 1) {
    ...
    if (input.enforcePromptRules !== false && type === "short" && countCharacters(text) > Math.min(180, input.maxLength)) {
      reject("TOO_LONG", "hard", `сократи короткий вариант до ${Math.min(180, input.maxLength)} символов`);
      continue;
    }
    if (input.enforcePromptRules !== false && type === "short" && countSentences(text) > 2) {
      reject("FORBIDDEN_PHRASE", "hard", "оставь в коротком варианте одно или два предложения");
      continue;
    }
    ...
    if (
      ...
      careerClichePatterns.some((pattern) => pattern.test(text)) &&
      !hardClichePatterns.some((pattern) => pattern.test(text))
    ) {
      reject(
        "FORBIDDEN_PHRASE",
        "soft",
    ...
    if (
      input.enforcePromptRules !== false &&
      genderedAuthorPattern.test(text) &&
      !genderedAuthorPattern.test(input.draftNotes)
    ) {
      reject("FORBIDDEN_PHRASE", "hard", "не придумывай пол автора; используй нейтральную формулировку без рад/рада или признателен/признательна");
      continue;
    }
    if (
      input.enforcePromptRules !== false &&
    ...
    if (input.enforcePromptRules !== false && type === "short" && repeatedWishCount >= 3) {
      reject("FORBIDDEN_PHRASE", "hard", "оставь в коротком варианте только одно главное пожелание");
      continue;
    }
    const hasDiplomaJoke = diplomaJokePattern.test(text);
    if (input.enforcePromptRules !== false && input.style !== "humor" && hasDiplomaJoke) {
      reject("FORBIDDEN_PHRASE", "hard", "не добавляй юмористическую фразу в выбранный стиль");
      continue;
    }
    if (input.enforcePromptRules !== false && input.style === "humor" && type !== "style" && hasDiplomaJoke) {
      reject("FORBIDDEN_PHRASE", "hard", "оставь шутку только в варианте выбранного стиля");
      continue;
    }
  ...
  if (variants.length > 1 && input.enforcePromptRules !== false) {
    const repeatedTypes = new Set<AiVariantType>();
    for (let index = 1; index < variants.length; index += 1) {
      for (let previousIndex = 0; previousIndex < index; previousIndex += 1) {
        const phrase = findSharedTemplatePhrase(variants[index].text, variants[previousIndex].text);
        if (!phrase) continue;
        repeatedTypes.add(variants[index].id);
        issues.push({
          type: variants[index].id,
          code: "TOO_SIMILAR",
          severity: "hard",
          message: `${variants[index].id}: не повторяй формулировку «${phrase}» из другого варианта`
        });
        break;
      }
    }
    if (repeatedTypes.size > 0) {
      return { variants: variants.filter((variant) => !repeatedTypes.has(variant.id)), issues };
    }
  }
