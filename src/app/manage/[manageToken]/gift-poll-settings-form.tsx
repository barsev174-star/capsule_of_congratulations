        ...
        <section className={styles.giftPollOptionsSection}>
          ...
          <div className={styles.giftPollOptionList}>
            {options.map((option, index) => {
              ...
              return <article key={option.id} draggable onDragStart={() => setDraggedOptionId(option.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedOptionId) moveOptionTo(draggedOptionId, option.id); setDraggedOptionId(null); }} onDragEnd={() => setDraggedOptionId(null)} className={`${styles.giftPollOptionEditor} ${isBudget ? styles.giftPollBudgetEditor : ""} ${draggedOptionId === option.id ? styles.giftPollOptionDragging : ""}`}>
                <input type="hidden" name="optionId" value={option.id} />
                <header className={styles.giftPollOptionCardHeader}>
                  <span className={styles.giftPollDragHint} aria-label="Перетащите, чтобы изменить порядок">⠿</span>
