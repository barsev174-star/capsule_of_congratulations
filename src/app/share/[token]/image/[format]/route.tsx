...
const PhotoCard = ({ photo, index, width, theme, origin, captionSize, format }: { photo: Payload["photos"][number]; index: number; width: number; theme: Theme; origin: string; captionSize: number; format: Format }) => {
  ...
  const captionBottom = route ? format === "story" ? "1.5%" : isCompactCaption ? "0.5%" : "2.5%" : format === "story" ? "5%" : "9%";
  ...
  return <div style={{ position: "relative", display: "flex", width, height, flexShrink: 0, transform: `rotate(${index === 1 ? 1.2 : index === 2 ? -1.2 : -1.5}deg)`, boxShadow: route ? "0 8px 16px rgba(0,0,0,.22)" : "none" }}>
    ...
    <img src={photo.url} style={{ position: "absolute", left: route ? "6%" : "9%", top: route ? "5.1%" : "13%", width: route ? "88%" : "82%", height: route ? "75.9%" : "58%", objectFit: "cover", ...(route ? { outline: "1px solid rgba(0,0,0,.1)" } : {}) }} />
    {!route ? <img src={asset(origin, frame)} style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} /> : null}
...
const ExportCard = ({ payload, format, origin }: { payload: Payload; format: Format; origin: string }) => {
  ...
  const moments = photos.length === 3 && format !== "post" ? <div style={{ position: "relative", display: "flex", flexDirection: "column", width: "100%" }}>
    <div style={{ display: "flex", alignSelf: "flex-start", ...(format === "story" ? { position: "absolute", left: 92, top: 24, zIndex: 1 } : { position: "absolute", left: 78, top: 52, zIndex: 1 }) }}>{momentHeading}</div>
    <div style={{ display: "flex", alignItems: format === "story" ? "flex-start" : "flex-end", justifyContent: "center", gap: 0, marginTop: format === "story" ? 12 : 24, width: "100%", ...(format === "story" ? { transform: "translateX(5px)" } : format === "print" ? { transform: "translateX(10px)" } : {}) }}>
      <div style={{ display: "flex", paddingTop: format === "story" ? 128 : 0, ...(format === "print" ? { transform: "translate(-78px,-8px) scaleX(1.05)" } : {}) }}><PhotoCard photo={photos[0]} index={0} width={paper ? Math.round(p.mainPhoto * (format === "print" ? 1 : 1.05)) : p.mainPhoto} theme={theme} origin={origin} captionSize={p.caption} format={format} /></div>
      <div style={{ display: "flex", flexDirection: "column", gap: 4, ...(format === "print" ? { transform: "translateX(-12px) scaleX(1.1)" } : {}) }}>{photos.slice(1).map((photo, index) => <PhotoCard key={photo.id} photo={photo} index={index + 1} width={paper ? Math.round(p.sidePhoto * (format === "print" ? 1 : 1.1)) : p.sidePhoto} theme={theme} origin={origin} captionSize={p.caption} format={format} />)}</div>
    </div>
  </div> : photos.length === 2 && format === "post" ? <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "center", gap: 6, width: "100%" }}>
    <div style={{ display: "flex", transform: paper ? "translateX(-12px)" : undefined }}><PhotoCard photo={photos[0]} index={0} width={paper ? Math.round(p.mainPhoto * 1.05) : p.mainPhoto} theme={theme} origin={origin} captionSize={p.caption} format={format} /></div>
