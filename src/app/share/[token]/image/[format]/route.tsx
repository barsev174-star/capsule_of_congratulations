...
const ExportCard = ({ payload, format, origin }: { payload: Payload; format: Format; origin: string }) => {
  ...
  const moments = photos.length === 3 && format !== "post" ? <div style={{ position: "relative", display: "flex", flexDirection: "column", width: "100%" }}>
    <div style={{ display: "flex", alignSelf: "flex-start", ...(format === "story" ? { position: "absolute", left: 28, top: -8, zIndex: 1 } : { position: "absolute", left: 42, top: 44, zIndex: 1 }) }}>{momentHeading}</div>
    <div style={{ display: "flex", alignItems: format === "story" ? "flex-start" : "flex-end", justifyContent: "center", gap: format === "story" || format === "print" ? 8 : 18, marginTop: format === "story" ? 12 : 24, width: "100%", ...(format === "story" ? { transform: "translateX(5px)" } : format === "print" ? { transform: "translateX(22px)" } : {}) }}>
      <div style={{ display: "flex", paddingTop: format === "print" ? 0 : format === "story" ? 128 : 10, ...(format === "print" ? { transform: "translateX(-67px)" } : {}) }}><PhotoCard photo={photos[0]} index={0} width={p.mainPhoto} theme={theme} origin={origin} captionSize={p.caption} format={format} /></div>
      <div style={{ display: "flex", flexDirection: "column", gap: format === "print" ? 6 : 12 }}>{photos.slice(1).map((photo, index) => <PhotoCard key={photo.id} photo={photo} index={index + 1} width={p.sidePhoto} theme={theme} origin={origin} captionSize={p.caption} format={format} />)}</div>
    </div>
  </div> : photos.length === 2 && format === "post" ? <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "center", gap: 12, width: "100%" }}>
    <PhotoCard photo={photos[0]} index={0} width={p.mainPhoto} theme={theme} origin={origin} captionSize={p.caption} format={format} />
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>{momentHeading}<PhotoCard photo={photos[1]} index={1} width={p.sidePhoto} theme={theme} origin={origin} captionSize={p.caption} format={format} /></div>
