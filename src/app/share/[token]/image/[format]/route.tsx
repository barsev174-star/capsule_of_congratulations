...
const PhotoCard = ({ photo, index, width, theme, origin, captionSize, format }: { photo: Payload["photos"][number]; index: number; width: number; theme: Theme; origin: string; captionSize: number; format: Format }) => {
  ...
  return <div style={{ position: "relative", display: "flex", width, height, flexShrink: 0, transform: rotation, boxShadow: route ? "0 8px 16px rgba(0,0,0,.22)" : "none" }}>
...
const ExportCard = ({ payload, format, origin }: { payload: Payload; format: Format; origin: string }) => {
  ...
  const moments = photos.length === 3 && format !== "post" ? <div style={{ position: "relative", display: "flex", flexDirection: "column", width: "100%" }}>
    <div style={{ display: "flex", alignSelf: "flex-start", ...(format === "story" ? { position: "absolute", left: 92, top: 38, zIndex: 1 } : { position: "absolute", left: 78, top: 52, zIndex: 1 }) }}>{momentHeading}</div>
